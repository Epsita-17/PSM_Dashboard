...
# IMPORTANT:
# These are the GIDs supplied for the CPP dashboard.
...
def load_module(name):
    """Load a module and then apply the CPP filter."""
    raw = load_google_sheet(SHEETS.get(name))
    return filter_CPP(raw)
...
st.markdown(
    f"""
    <div class="live-bar">
        <b>LIVE DATA: Google Sheet → CPP</b>
        &nbsp; | &nbsp;
        Refresh: 5 minutes
        &nbsp; | &nbsp;
            ...
            with st.container(
                ...
                if (
                    ...
                    if not category_counts.empty:
                        ...
                        st.info(
                            "No CPP MOC Category data found."
                        )
