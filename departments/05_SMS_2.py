...
# ============================================================
st.set_page_config(
    page_title="PSM Dashboard - SMS-2",
    page_icon="🔥",
    layout="wide",
...
# IMPORTANT:
# These are the GIDs supplied for the SMS-2 dashboard.
...
def load_module(name):
    """Load a module and then apply the SMS-2 filter."""
    raw = load_google_sheet(SHEETS.get(name))
    return filter_SMS_2(raw)
...
def show_register(title, df, id_names, description_names, status_names):
    ...
    if register_df.empty:
        ...
        st.info("No SMS-2 records found.")
...
st.markdown(
    f"""
    <div class="live-bar">
        <b>LIVE DATA: Google Sheet → SMS-2</b>
        &nbsp; | &nbsp;
        Refresh: 5 minutes
        &nbsp; | &nbsp;
            ...
            with st.container(
                ...
                if (
                    ...
                    if not category_counts.empty:
                        ...
                        st.info(
                            "No SMS-2 MOC Category data found."
                        )
