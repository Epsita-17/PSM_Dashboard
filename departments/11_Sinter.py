...
# ============================================================
st.set_page_config(
    page_title="PSM Dashboard - Sinter Plant",
    page_icon="🔥",
    layout="wide",
...
# IMPORTANT:
# These are the GIDs supplied for the Sinter Plant dashboard.
...
def filter_sinter_plant(df):
    """
    Filter only Sinter Plant records.
...
def load_module(name):
    """Load a module and then apply the Sinter Plant filter."""
    raw = load_google_sheet(SHEETS.get(name))
    return filter_sinter_plant(raw)


def find_status_col(df, candidates=None):
    """Find the REAL status column, prioritizing explicit Open/Close status."""
    if df is None or df.empty:
        return None
    ...
    for column in df.columns:
        ...
        if any(k in key for k in [
            "status", "actionstatus", "completionstatus",
            "investigationstatus", "recommendationstatus"
        ]):
            return column
...
def show_register(title, df, id_names, description_names, status_names):
    ...
    if register_df.empty:
        ...
        st.info("No Sinter Plant records found.")
...
audit = filter_sinter_plant(audit)
...
st.markdown(
    f"""
    <div class="live-bar">
        <b>LIVE DATA: Google Sheet → Sinter Plant</b>
        &nbsp; | &nbsp;
        Refresh: 5 minutes
        &nbsp; | &nbsp;
...
# SINTER — FINAL HORIZONTAL LAYOUT
# ============================================================
            ...
            with st.container(
                ...
                if (
                    ...
                    if not category_counts.empty:
                        ...
                        st.info(
                            "No SINTER MOC Category data found."
                        )
...
with c:
    ...
    with st.container(
        ...
        if (
            ...
            st.info(
                "No Sinter Plant training data found."
            )
