...
# IMPORTANT:
# These are the GIDs supplied for the CRM dashboard.
...
def load_module(name):
    """Load a module and then apply the CRM filter."""
    raw = load_google_sheet(SHEETS.get(name))
    return filter_CRM(raw)


def find_status_col(df, candidates=None):
    """Find the actual status column, prioritizing explicit status fields."""
    if df is None or df.empty:
        return None
...
st.markdown(
    f"""
    <div class="live-bar">
        <b>LIVE DATA: Google Sheet → CRM</b>
        &nbsp; | &nbsp;
        Refresh: 5 minutes
        &nbsp; | &nbsp;
            ...
            with st.container(
                ...
                if (
                    ...
                    if not category_counts.empty:
                        ...
                        st.info(
                            "No CRM MOC Category data found."
                        )
